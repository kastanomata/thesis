...
% Le conclusioni dello studio ``Dynamic Storage Allocation, A Survey and Critical Review'' sull'argomento sono riassunte dagli autori. Le riportiamo perché sembrano quanto mai rilevanti ad acquisire una prospettiva corretta sui risultati ottenuti. Innanzitutto, il comportamento dei programmi è tipicamente variabile nel tempo, anziché costante. In secondo luogo, l'utilizzo massimo della memoria (\textit{peak memory usage}) è un fattore critico, poiché la frammentazione in questi momenti di picco è più significativa rispetto alle fasi intermedie. La frammentazione è causata proprio da questo comportamento dinamico, specialmente quando i picchi di utilizzo coinvolgono oggetti di dimensioni diverse. La conoscenza del comportamento specifico di un programma può invalidare risultati sperimentali e analitici precedenti: il comportamento non casuale dei programmi può essere sfruttato per ottimizzarne le prestazioni. Infine, programmi diversi possono mostrare modelli caratteristici e non casuali, che variano in base alla loro natura e funzionalità. \textbf{Questi aspetti sottolineano l'importanza di un'analisi personalizzata per migliorare l'efficienza nella gestione della memoria.}
